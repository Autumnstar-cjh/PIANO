...
import matplotlib.pyplot as plt # edited by Yutao
...
class Trainer:
    def __init__(self, *, 
                 ...
                 verbose=False,
                 save_checkpoint_interval=None):  # Add save_checkpoint_interval parameter(Yutao)
        """
        ...
        self.save_checkpoint_interval = save_checkpoint_interval  #Yutao
    ...
    def train(self, train_loader, test_loaders,
            optimizer, scheduler, regularizer,
              training_loss=None, eval_losses=None, save_checkpoint_interval=100): # Added parameter for checkpoint saving(Yutao)
        ...
        #train_losses = [] # edited by Yutao
        epoch_losses = [] # edited by Yutao
        ...
        for epoch in range(self.n_epochs):
            ...
            for idx, sample in enumerate(train_loader):
                ...
                loss = loss.mean() # edited by Yutao
                #loss = torch.sum(loss)
                #print(loss) # edited by Yutao
                loss.backward()
                ...
                #train_losses.append(loss.item()) # edited by Yutao
            ...
            epoch_losses.append(train_err) # edited by Yutao
            ...
            if epoch % self.log_test_interval == 0: 
                ...
                for loader_name, loader in test_loaders.items():
                    ...
                    eval_results = self.evaluate(eval_losses, loader, log_prefix=loader_name)
                    
                    test_h1 = eval_results.get(f'{loader_name}_h1', torch.tensor(0.0)).mean().item()  #  edited by Yutao
                    test_l2 = eval_results.get(f'{loader_name}_l2', torch.tensor(0.0)).mean().item()  #  edited by Yutao
                    
                    print(f"[{epoch}] time={epoch_train_time:.2f}, avg_loss={avg_loss:.4f}, "
                          f"train_err={train_err:.4f}, {loader_name}_h1={test_h1:.4f}, {loader_name}_l2={test_l2:.4f}")
                    

                if self.callbacks:
                    self.callbacks.on_val_end()
                
            if self.save_checkpoint_interval is not None and epoch % self.save_checkpoint_interval == 0 and epoch > 0:      # edited by Yutao
                # Save the model every save_checkpoint_interval epochs
                checkpoint_path = f"/mmfs1/home/yd288/FNO/result/checkpoint_epoch_{epoch}.pt"
        ...
        plt.plot(epoch_losses, label='Train Loss') # edited by Yutao
        plt.xlabel('Epoch') # edited by Yutao
        plt.ylabel('Loss') # edited by Yutao
        plt.legend() # edited by Yutao
        plt.show() # edited by Yutao
